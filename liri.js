...
function UserInputs(userOption, inputParameter) {
  switch (userOption) {
    ...
    default:
      console.log(
        "Ruh Roh, An Error occurred. Please type any of the following options: \nconcert-this \nspotify-this-song \nmovie-this \ndo-what-it-says"
      );
  }
}
...
function showConcertInfo(inputParameter) {
  ...
  request(queryUrl, function(error, response, body) {
    ...
    if (!error && response.statusCode === 200) {
      ...
      console.log("Ruh Roh, An Error occurred.");
    }
  });
}
...
function showSongInfo(inputParameter) {
  if (inputParameter === undefined) {
    inputParameter = "Get Jiggy With It"; //default Song
  }
  spotify.search(
    {
      type: "track",
    ...
    function(err, data) {
      if (err) {
        console.log("Ruh Roh, An Error occurred: " + err);
        return;
      }
